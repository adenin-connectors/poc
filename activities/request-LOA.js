  ...
  try {
    ...
    switch (activity.Request.Path) {
      ...
      case "submit":
        ...
        var comment = T(activity, "Leave request {0} submitted.", randomId);
      ...
      default:
        var fname = __dirname + path.sep + "common" + path.sep + "request-LOA.form";
        var schema = yaml.safeLoad(fs.readFileSync(fname, 'utf8'));

        data.title = T(activity, "Leave Request");
        ...
        data._actionList = [{
          id: "create",
          label: T(activity, "Leave Request"),
          settings: {
            actionType: "a"
          }
        }];
