  ...
  try {
    ...
    switch (activity.Request.Path) {
      ...
      case "submit":
        ...
        var comment = T(activity, "Catering order {0} placed.", randomId);
      ...
      default:
        var fname = __dirname + path.sep + "common" + path.sep + "order-catering.form";
        var schema = yaml.safeLoad(fs.readFileSync(fname, 'utf8'));

        data.title = T(activity, "Order Catering");
        ...
        data._actionList = [{
          id: "create",
          label: T(activity, "Order Catering"),
          settings: {
            actionType: "a"
          }
        }];
